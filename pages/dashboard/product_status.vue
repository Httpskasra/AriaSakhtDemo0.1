  ...
  <section class="product-status-page" dir="rtl">
    ...
    <template v-else>
      ...
      <PanelDataTable v-else :rows="filteredProducts" :columns="[
        ...
        <template #status-data="{ row }"><AppSelect v-if="canUpdate" :model-value="row.status" size="xs" :items="productStatusOptions" :disabled="loadingProductId === (row._id || row.id)" @update:model-value="(value) => updateStatus(row, value as Product['status'])" /><StatusPill v-else :label="statusFa(row.status)" :semantic="productStatusSemantic(row.status)" size="compact" /></template>
