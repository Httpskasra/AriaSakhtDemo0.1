...
<style>
.panel-layout { --panel-sidebar-current-width:var(--panel-sidebar-width); min-height:100dvh; max-width:100%; overflow-x:clip; display:grid; grid-template-columns:minmax(0, 1fr) var(--panel-sidebar-current-width); grid-template-rows:auto minmax(0, 1fr); grid-template-areas:"header header" "main drawer"; direction:ltr; background:var(--color-bg-light); }
